...
function mod:CheckBGHealers()
	...
	for i = 1, GetNumBattlefieldScores() do
		...
		if(name) then
			...
			if(self.db.markFactionHealers == "ENEMY") then
			...
			elseif(self.db.markFactionHealers == "FRIENDLY") then
				if(name and self.HealerSpecs[talentSpec] and self.factionOpposites[self.PlayerFaction] ~= faction) then
					self.Healers[name] = talentSpec
				elseif(name and self.Healers[name]) then
					self.Healers[name] = nil;
				end
			elseif(self.db.markFactionHealers == "BOTH") then
